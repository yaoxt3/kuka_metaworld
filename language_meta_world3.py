...
from learning_to_be_taught.environments.meta_world.scripted_policies_v2 import policies2
...
class LanguageMetaWorld3(GeneralizedMetaWorld):
    EMBED_DIM = 50
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.has_antonym = False
        self.generate_oppo_sample = False
        self.timeout = False
        self.trial_success_reverse = False
        self.reverse_index = 0
        self.max_reward = 0
        self.reverse_action = []
        self.recover_mode = False
        self.recover_step_by_step = False
    ...
    def _reset_env(self, *args, instruction=None, **kwargs):
        ...
        self.trial_success_reverse = False
        self.recover_mode = False
        # print(f"full instruction: {self.full_instruction}")
    ...
    # the step function returns 'done' after the agent has finished three trials,
    # no matter whether the agent solves the task or not
    def step(self, action):
        ...
        if self.demonstration_phase:
            ...
            info = {}
            # oppo_inst = OpposedLanguageInstruction(self.full_instruction)
            # self.has_antonym = oppo_inst.has_antonym()
            
            if self.full_instruction.find('open') >= 0 and self.full_instruction.find('close') >= 0:
                self.has_antonym = True
            else:
                self.has_antonym = False
            self.has_antonym = True
            # print(f"has antonym: {self.has_antonym}")
        else:
            ...
            if self.generate_oppo_sample is False:
                for i in range(self.action_repeat):  # action repeat := 2
                    ...
                    self.valid_sample_one_trial.append(action)
            else:  # generate opposite samples using reverse actions and reward generated in the previous valid sample
                if not self.recover_mode:
                    if self.reverse_index >= 0:
                        ...
                        current_reward = 2*self.max_reward - reward
                        ...
                        if abs(current_reward-self.max_reward)/self.max_reward > 0.9 and self.trial_success_reverse:
                    ...
                    if self.recover_step_by_step:
                        r_action, finished = policies2['drawer-handle-v2']().get_action(self.observation)
                        self.observation, reward, done, info = self.env.step(r_action)
                        self.timeout = finished
                        reward_sum += reward
                    else:
                        length = self.env.get_curr_path_length()
                        self.env.set_curr_path_length(0)
                        finished = False
                        policy = policies2['drawer-handle-v2']()
                        o_count = 0
                        while not self.timeout:
                            r_finger, l_finger = self.env.get_finger_pos()
                            r_action = policy.get_action(self.observation)
                            self.observation, reward, done, info = self.env.step(r_action)
                            print(f"rfinger:{r_finger}, lfinger:{l_finger}")
                            print(f"obs:{self.observation[0:7]}")
                            print("-------------")
                            if (r_finger[1] < self.observation[5]) and (l_finger[1] > self.observation[5]) and \
                                    (l_finger[0] < self.observation[4]+0.02) and (l_finger[0] > self.observation[4]-0.2) and \
                                    abs(self.observation[2] - self.observation[6]) <= 0.01:
                                o_count += 1
                                print("count+1")
                                if o_count > 10:
                                    self.timeout = True
                            ...
                            self.env.render()
                            time.sleep(0.03)
                        self.env.set_curr_path_length(length)

            # which indicates this trial is finish, success or fail
            if self.trial_timeout() or self.timeout:
                # if this trial is success and the language instruction has antonyms,
                # the corresponding opposite samples will be generated step by step
                if self.trial_success and self.has_antonym and not self.generate_oppo_sample:
                    self.generate_oppo_sample = True

                    self.trial_success_reverse = self.trial_success
                    self.step_in_trial = 0
                    self.trial_success = False
                    self.reverse_index = np.shape(self.valid_sample_one_trial)[0] - 1
                    self.reverse_action = self.valid_sample_one_trial[::-1]
                    self.reverse_action = np.negative(self.reverse_action)  # negative
                    self.reverse_action[:, 2:4] = np.negative(self.reverse_action[:, 2:4])
                    r_finger, l_finger = self.env.get_finger_pos()
                    if (r_finger[1] < self.observation[5]) and (l_finger[1] > self.observation[5]) and \
                            abs(self.observation[2] - self.observation[6]) <= 0.03 and \
                            (l_finger[0] < self.observation[4]+0.02) and (l_finger[0] > self.observation[4]-0.02):
                        self.recover_mode = False
                    else:
...
env_instructions = {
    ...
    "reach-v2": ["reach", 'Lift up claw'],
    'push-v2': ["push", 'push object forward', 'push object forward with claw'],
    'pick-place-v2': ["pick and place", 'pick object and then place'],
    'door-open-v2': ['open door', "pull back door", "open door and then close door"],
    'drawer-close-v2': ["close drawer", "close drawer and then open drawer", "push forward drawer", "push forward drawer and then pull back drawer"],
    'button-press-topdown-v2': ['press button', 'press object down'],
    'peg-insert-side-v2': ["pick and insert", "pick object and place from left"],
    'window-open-v2': ['open window', 'sweep window handle from left', 'sweep object from left to right'],
    'basketball-v2': ['pick and place', 'pick ball and place', 'pick ball and place from the top'],
    'sweep-v2': ['sweep', 'sweep block from table', 'slide object from left to right'],

    'drawer-open-v2': ["open drawer", 'pull back the drawer'],
    'door-close-v2': ['close door'],
    'sweep-into-v2': ['push object forward', 'push block into hole'],
    'shelf-place-v2': ['pick and place', 'pick object and then place on shelf'],
    'lever-pull-v2': ['push object up', 'push object and Lift up claw'],

    'window-close-v2': ['push left to goal_pos', 'push object left', 'slide object right', 'close window'],
    'reach-wall-v2': ['reach over obstacle to goal_pos', 'reach goal_pos', 'reach to goal_pos'],
    'pick-place-wall-v2': ['pick object and place at goal_pos', 'pick and place at goal_pos'],
    'push-wall-v2': ['push object around obstacle to goal_pos', 'push to goal_pos', 'push object to goal_pos'],
    'button-press-v2': ['press forward', 'press button forward', 'push to goal_pos'],
    'button-press-topdown-wall-v2': ['press behind obstacle', 'press down to goal_pos', 'press down'],
    'button-press-wall-v2': ['press button behind obstacle', 'press forward', 'press forward to goal_pos'],
    'peg-unplug-side-v2': ['pull object to right', 'pull object to goal_pos', 'pick object and pull to right'],
    'disassemble-v2': ['pick and pull up', 'pick and place at goal_pos', 'pick and put down at goal_pos'],
    'hammer-v2': ['push to goal_pos with object', 'push to goal_pos with hammer', 'use object to push to goal_pos'],
    'plate-slide-v2': ['push to goal_pos', 'push plate to goal_pos', 'push forward to goal_pos'],
    'plate-slide-side-v2': ['push left to goal_pos', 'push plate left to goal_pos', 'slide left to goal_pos'],
    'plate-slide-back-v2': ['push back to goal_pos', 'push plate back to goal_pos', 'slide back to goal_pos'],
    'plate-slide-back-side-v2': ['push right to goal_pos', 'push plate right to goal_pos', 'slide right to goal_pos'],
    'handle-press-v2': ['push down', 'press down', 'push down to goal_pos', 'press down to goal_pos'],
    'handle-pull-v2': ['pull up', 'push up', 'push up to goal_pos', 'pull up to goal_pos'],
    'handle-press-side-v2': ['push down', 'press down', 'push down to goal_pos', 'press down to goal_pos'],
    'handle-pull-side-v2': ['pull up', 'push up', 'push up to goal_pos', 'pull up to goal_pos'],
    'stick-push-v2': ['push with object to goal_pos', 'use stick to push to goal_pos', 'push with stick to goal_pos'],
    'stick-pull-v2': ['push with object to goal_pos', 'use stick to push to goal_pos', 'push with stick to goal_pos'],
    'soccer-v2': ['push to goal_pos', 'push ball to goal_pos', 'place object at goal_pos', 'place ball at goal_pos'],
    'faucet-open-v2': ['open faucet', 'push to goal_pos', 'push right', 'push from left to right',
                       'turn object from left to right', 'turn to goal_pos'],
    'faucet-close-v2': ['close faucet', 'push to goal_pos', 'push left', 'push from right to left',
                        'turn object from right to left', 'turn to goal_pos'],
    'coffee-push-v2': ['push object to goal_pos', 'push cup to goal_pos', 'push object forward to goal_pos'],
    'coffee-pull-v2': ['place object away', 'pick and place at goal_pos', 'place cup away'],
    'coffee-button-v2': ['push forward', 'press button', 'press coffee button'],
    'pick-out-of-hole-v2': ['pick object', 'pick object out of hole', 'pick and place at goal_pos'],
    'assembly-v2': ['pick and place at goal_pos', 'assemble', 'pick object and place down at goal_pos'],
    'push-back-v2': ['push back to goal_pos', 'push to goal_pos', 'puch block to goal_pos',
                     'push block back to goal_pos'],
    'dial-turn-v2': ['turn object', 'dial turn', 'rotate object'],
    'bin-picking-v2': ['pick object', 'pick and place at goal_pos'],
    'box-close-v2': ['pick object and place at goal_pos', 'pick and place at goal_pos'],
    'hand-insert-v2': ['sweep object to goal_pos', 'pick object and place at goal_pos', 'push object to goal_pos',
                       'push block to goal_pos'],
    'door-lock-v2': ['push object down to goal_pos', 'turn object down to goal_pos'],
    'door-unlock-v2': ['push object to goal_pos', 'turn object to goal_pos'],
}
...
if __name__ == '__main__':
    ...
    env = LanguageMetaWorld3(benchmark='ml10', action_repeat=1, demonstration_action_repeat=5,
