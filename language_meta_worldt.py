...
# tset the opposition-sample generating function
class LanguageMetaWorldT(GeneralizedMetaWorld):
    EMBED_DIM = 50
        ...
        if self.demonstration_phase:
            ...
            if self.generate_oppo_sample is False:
                for i in range(self.action_repeat):  # action repeat := 2
                    ...
                    self.valid_sample_one_trial.append((reward, action))
            ...
            # if self.trial_success or self.trial_timeout() or self.timeout:
            if self.trial_timeout() or self.timeout:
                ...
                if self.trial_success and self.has_antonym and (self.generate_oppo_sample is False):
                    ...
                    self.reverse_index = len(self.valid_sample_one_trial) - 1
                    self.reverse_action = np.array(self.valid_sample_one_trial)[:, -1].tolist()
...
env_instructions = {
    ...
    'basketball-v1': ['pick and place at goal_pos', 'pick ball and place at goal_pos',
                      'pick ball and place at goal_pos from the top'],
    ...
    'sweep-v1': ['sweep', 'sweep object from table', 'sweep block from table', 'slide object from table',
                 'slide block from table'],
    ...
    'shelf-place-v1': ['pick and place at goal_pos', 'place on shelf at goal_pos',
                       'pick object and place on shelf at goal_pos'],
