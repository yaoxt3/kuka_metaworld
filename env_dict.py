...
from metaworld.envs.mujoco.sawyer_xyz.kuka import SawyerReachPushPickPlaceKukaS2REnv
...
def _single_medium_mode_args_kwargs(env_cls_, key_):
    del env_cls_
    kwargs = dict(task_id=list(ALL_V1_ENVIRONMENTS.keys()).index(key_))
    return dict(args=[], kwargs=kwargs)

SIGNLE_MEDIUM_MODE_ARGS_KWARGS = dict(train={})
for key, env_cls in HARD_MODE_CLS_DICT['train'].items():
    SIGNLE_MEDIUM_MODE_ARGS_KWARGS['train'][key] = _single_medium_mode_args_kwargs(env_cls, key)
