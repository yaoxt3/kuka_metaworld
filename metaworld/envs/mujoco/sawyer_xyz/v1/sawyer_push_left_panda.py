...
class SawyerPushLeftEnvPanda(SawyerXYZEnv):

    def __init__(self):
        liftThresh = 0.04
        goal_low = (-0.1, 0.6, 0.02)
        goal_high = (0., 0.7, 0.02)
        hand_low = (-0.2, 0.40, 0.05)
        hand_high = (0.2, 0.75, 0.3)
        obj_low = (0., 0.5, 0.02)
        obj_high = (0., 0.5, 0.02)

        # x y exchange
        # goal_low = (0.6, -0.1, 0.02)
        # goal_high = (0.7, 0., 0.02)
        # hand_low = (0.40, -0.2, 0.05)
        # hand_high = (0.75, 0.2, 0.3)
        # obj_low = (0.5, 0., 0.02)
        # obj_high = (0.5, 0., 0.02)

        # x y exchange, -x
        # goal_low = (-0.7, -0.1, 0.02)
        # goal_high = (-0.6, 0., 0.02)
        # hand_low = (-0.7, -0.2, 0.05)
        # hand_high = (-0.4, 0.2, 0.3)
        # obj_low = (-0.5, 0., 0.02)
        # obj_high = (-0.5, 0., 0.02)
    ...
    def _set_task_inner(self, *, task_type, **kwargs):
        ...
        if self.task_type == 'pick_place':
            self.goal = np.array([0.1, 0.7, 0.2])
        elif self.task_type == 'reach':
            self.goal = np.array([-0.1, 0.7, 0.2])
        elif self.task_type == 'push':
            self.goal = np.array([-0.1, 0.7, 0.02])
        # elif self.task_type == 'push':
    ...
    def model_name(self):
        ...
        return full_v1_path_for('sawyer_xyz/sawyer_reach_push_pick_and_place_panda.xml')
    ...
    def compute_reward(self, actions, obs):
        ...
        def compute_reward_push(actions, obs):
            ...
            del actions
            del obs
            print(f"l:{leftFinger}, r:{rightFinger}")
            print(f"obj:{objPos}, finger:{fingerCOM}")
            print("-------------------------")
