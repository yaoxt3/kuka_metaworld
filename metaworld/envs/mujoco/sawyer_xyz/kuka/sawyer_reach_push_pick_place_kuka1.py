...
class SawyerReachPushPickPlaceKukaEnv(SawyerXYZEnv):

    def __init__(self):
        liftThresh = 0.04
        goal_low = (-0.1, 0.65, 0.05)
        goal_high = (0.1, 0.75, 0.2)
        hand_low = (-0.45, 0.45, 0.05)
        hand_high = (0.45, 0.85, 0.3)
        obj_low = (-0.1, 0.45, 0.02)
        obj_high = (0.1, 0.55, 0.02)
        ...
        self.init_config = {
            'obj_init_angle': .3,
            'obj_init_pos': np.array([0, 0.45, 0.02]),
            'hand_init_pos': np.array([0, 0.45, 0.2]),
        }
    ...
    def _set_task_inner(self, *, task_type, **kwargs):
        ...
        if self.task_type == 'pick_place':
            self.goal = np.array([0.1, 0.65, 0.2])
        elif self.task_type == 'reach':
            self.goal = np.array([-0.1, 0.65, 0.2])
        elif self.task_type == 'push':
            self.goal = np.array([0.1, 0.65, 0.02])
        else:
